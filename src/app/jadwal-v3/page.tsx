...
export default function JadwalV3Page(): React.JSX.Element {
                  ...
                  return (
                    <tr key={rowKey} className={rowClassName}>
                      ...
                      {classrooms.map((classroom) => {
                        ...
                        let isHighlightRed = false;

                        if (draggingPayload && !blocked) {
                          const isTargetClass = (draggingPayload.kind === "assignment" ? draggingPayload.classroomId : draggingPayload.sourceClassroomId) === classroom.id;

                          if (isTargetClass) {
                            const draggedTeacherId = draggingPayload.kind === "assignment" ? draggingPayload.teacherId : draggingPayload.sourceTeacherId;
                            
                            const teacherBusyHere = entries.some((e) => {
                              if (e.day !== day || e.timeSlotId !== timeSlot.id) return false;
                              if (e.teacherId !== draggedTeacherId) return false;
                              if (draggingPayload.kind === "entry" && e.id === draggingPayload.entryId) {
                                return false; // Not a conflict with itself
                              }
                              return true;
                            });
                        ...
                        let dropBg = "#fff";
                        if (blocked) dropBg = "#fff4ef";
                        else if (isHighlightRed) dropBg = "#fbeae8";
                        else if (isHighlightGreen) dropBg = "#e6f4ea";
